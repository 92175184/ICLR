...
async def run_tool_enabled_llm(
    ...
    """Run question through LLM with access to all tools"""
    try:
        # Get LLM
        llm = get_llm_by_model(model_name)

        # Create anonymized tools introduction for LLM decision-making
        tools_intro = """
You have access to comprehensive crypto analysis tools. Analyze the question and decide which tools to use based on the question's requirements.
...
**Instructions:**
- Analyze the question carefully and select the most appropriate tools
- For complex analysis, combine relevant tools strategically
- Always provide a complete answer based on the tool results
- Use tools that best match the question's data requirements
"""

        # Create reasoning prompt that asks for structured JSON output
        reasoning_prompt = f"""You are a crypto specialist with access to comprehensive tools. Your job is to analyze questions and use the most appropriate tools to provide accurate answers.

{tools_intro}

**Critical Instructions:**
- SELECTIVELY choose tools based on the question's requirements
- Always provide a complete, accurate answer based on tool results
- Do NOT execute all available tools - be selective and strategic
- After using tools, provide a clear final answer with calculations, percentages, or specific values
- Do NOT just repeat raw tool data - synthesize and calculate the final result

**IMPORTANT: You must respond with a JSON object containing two fields:**
- "reasoning": Your step-by-step analysis and thought process
- "answer": The specific final result (number, percentage, address, etc.)

Question: {question}

Please analyze the question and use the appropriate tools to gather information, then provide your response in this exact JSON format:
{{
  "reasoning": "Your detailed analysis and reasoning process here...",
  "answer": "The specific final result here"
}}"""

        # Bind tools to LLM and create messages
        ...
        # Run LLM with tools in a loop to handle tool execution
        max_iterations = 5
        iteration = 0
        all_tool_results = []
        initial_reasoning = ""  # Store initial reasoning from first response
        ...
        while iteration < max_iterations:
            ...
            # Capture initial reasoning from first response (if no tools were called yet)
            if iteration == 1 and not all_tool_results:
                response_text = _extract_text_from_content(
                    getattr(response, "content", "")
                )
                initial_reasoning, _ = _parse_structured_response(response_text)
                if not initial_reasoning:
                    initial_reasoning = response_text  # Fallback to full response
            ...
            if hasattr(response, "tool_calls") and response.tool_calls:
                ...
                for tool_call in response.tool_calls:
                    ...
                    # Store tool result for final synthesis
                    all_tool_results.append(f"{tool_name}: {str(tool_result)}")
            else:
                # No more tool calls, break the loop
                _logger.info(
                    f"  ✅ LLM completed tool usage after {iteration} iterations"
                )
                break
        ...
        if all_tool_results:
            ...
            # Create synthesis prompt with all tool results and initial reasoning
            tool_results_text = "\n".join(all_tool_results)
            synthesis_prompt = HumanMessage(
                content=f"""
Based on the following initial analysis and tool results, please provide a clear, direct answer to the question: "{question}"
...
Please synthesize this information and provide a comprehensive answer that:
- Builds upon the initial analysis
- Incorporates the tool results effectively
- Directly addresses the question asked
- Includes relevant numerical data and calculations
- Provides specific values, percentages, or ratios where applicable
- Synthesizes everything into a clear, final answer

**IMPORTANT: You must respond with a JSON object containing two fields:**
- "reasoning": Combine the initial analysis with tool results analysis and show how you arrived at your conclusion
- "answer": The specific final result (number, percentage, address, etc.)

Please provide your response in this exact JSON format:
{{
  "reasoning": "Your comprehensive analysis combining initial reasoning and tool results here...",
  "answer": "The specific final result here"
}}
"""
            )
